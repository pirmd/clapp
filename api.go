...
// manner commands with their flags, arguments and sub-commands.
...
// Argument's type (int64, string, strings, bool) is guessed after the linked
// variable type.
...
// NewBoolFlagToVar creates a new boolean flag that is linked to the given var
...
// NewStringsFlag creates a new strings (slice of string) flag. This flag is
// cumulative in that it accepts an unlimited list of string to build flag,
// string are separated by ',' and therefore cannot contain ','.
func (c *Command) NewStringsFlag(name, usage string) *[]string {
	p := new([]string)
	c.NewFlagToVar(p, name, usage)
	return p
}

// NewStringsFlagToVar creates a new strings (slice of string) flag that is
// linked to the given var.
func (c *Command) NewStringsFlagToVar(p *[]string, name, usage string) {
	c.NewFlagToVar(p, name, usage)
}
...
// NewStringsArg creates a new strings (slice of strings) argument.
// This argument is cumulative in that it will consume all the remaining
// command line arguments to feed a slice of strings. It shall be the last
// argument of the command otherwise command line parsing will panic.
...
// NewStringsArgToVar creates a new strings (slice of strings) argument that is
...
// the last argument of the command otherwise command line parsing will panic.
